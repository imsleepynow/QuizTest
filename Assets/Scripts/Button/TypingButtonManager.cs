...
public class TypingButtonManager : MonoBehaviour
{
    ...
    public void TypingButtonAppear(string type)
    {
        ...
        for (int i = 0; i < nowType.Count; i++)
        {
            ...
            var but = Instantiate(button, buttonSet.transform);
            ...
            var world = new Vector3(hidariSpace + hidariue.x + yoko * buttonSize, hidariue.y + tate * buttonSize, 0);
            butTransform.position = buttonSet.transform.TransformPoint(world);
            buttons.Add(but);
        ...
        {
            var delButton = Instantiate(button, buttonSet.transform);
            var butTransform = delButton.GetComponent<RectTransform>();
            var world = new Vector3(hidariSpace + hidariue.x + buttonSize * 13, hidariue.y + 1.5f * buttonSize, 0);
            butTransform.position = buttonSet.transform.TransformPoint(world);
        ...
        {
            var retButton = Instantiate(button, buttonSet.transform);
            var butTransform = retButton.GetComponent<RectTransform>();
            var world = new Vector3(hidariSpace + hidariue.x + buttonSize * 13, hidariue.y + 0.25f * buttonSize, 0);
            butTransform.position = buttonSet.transform.TransformPoint(world);
